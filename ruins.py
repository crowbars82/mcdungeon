...
class StepPyramid(Blank):
    ...
    def render (self):
        ...
        mat = materials.StoneSlab #WoodenSlab [cb82] Wood can burn, even by mistake, and if player has no blocks... yet...
        if random.randint(1,100) <= 50:
            mat = materials.StoneSlab
        for p in iterate_spiral(Vec(start.x+1,7,start.z+1),
                                Vec(start.x+5,7,start.z+5),
                                (abs(c1.y)+3)*2-8): #  (abs(c1.y)+3)*2): [cb82] bad maths guesses
            self.parent.parent.setblock(Vec(p.x,
                                        0+int(p.y-8/2), # 0+int(p.y/2), [cb82] when regenerating a map, the stairs break.
...
class RoundTowerEntrance(Blank):
    ...
    def render (self):
        ...
        for p in iterate_spiral(Vec(start.x,room_floor+4,start.z),
                                ...
                                (room_floor-blev)*2-8): #(room_floor-blev)*2): [cb82] Lab-dog. just try a -8 there... seems to work.
            self.parent.parent.setblock(Vec(p.x,
                                        (p.y-8)/2, #p.y/2, [cb82] I am learning though... so hopefully getting somewhere! :D
                                        p.z), mat, mat.data+((p.y&1)^1)*8)
        # Supply chest
...
class SquareTowerEntrance(Blank):
    ...
    def render (self):
        ...
        for p in iterate_spiral(Vec(start.x,room_floor+4,start.z),
                                Vec(start.x+4,room_floor+4,start.z+4),
                                (room_floor-blev)*2): #(room_floor-blev)*2): [cb82]
            self.parent.parent.setblock(Vec(p.x,
                                        p.y/2, #p.y/2, [cb82]
                                        p.z), mat, mat.data+((p.y&1)^1)*8)
        # Supply chest
