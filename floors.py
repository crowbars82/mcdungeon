            ...
            for x in iterate_points_inside_flat_poly(*self.parent.canvas):
                if ((x.x+x.z)&1 == 1):
                    self.parent.parent.setblock(x+self.parent.loc,
                                                materials.LapisBlock) # materials.Wood) [cb82] lavachasms disagree with these I thought but that's me
                else:
                    self.parent.parent.setblock(x+self.parent.loc,
                                                materials.EmeraldBlock) #materials.WoodPlanks) [cb82]
...
class Bridges(Blank):
    ...
    sandpit = False
    slabtypes = (
        materials.Air, #materials.WoodenSlab, [cb82] having giant pits + using found enderpearls and ladders is quite fun
        materials.Air, #materials.SpruceSlab, [cb82]
        materials.Air,  #materials.BirchSlab, [cb82]
        materials.StoneBrickSlab  #materials.JungleSlab [cb82] 
        )
    def render(self):
        pn = perlin.SimplexNoise(256)
